...
Matrix4F Camera::get_view_projection() {
    ...
    //camera_rot.init_rotation(V4F(0,-1,1).normalized(), V4F(0, 1, 1).normalized(), V4F(1, 0, 0).normalized());
    //camera_rot.init_rotation(V4F(0,-1,1), V4F(0, 1, 1));
    camera_rot = m_transform.rot.to_rotation_matrix();
    Quaternion test = Quaternion(V4F(0, 0, 1), 30.0f);

    return  camera_trans * test.to_rotation_matrix() * m_projection;
}
