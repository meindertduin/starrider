...
struct PPIINVZBEdge {
    ...
    PPIINVZBEdge() = default;

    PPIINVZBEdge(const Vertex4D &min_y_vert, const Vertex4D &max_y_vert) {
...
void scan_edges(PPIINVZBEdge &long_edge, PPIINVZBEdge &short_edge, bool handedness, A565Color color, const RenderListPoly &poly) {
    float *iz_ptr;
    int y_start = short_edge.y_start;
    int y_end = short_edge.y_end;
    ...
    PPIINVZBEdge &left = handedness ? short_edge : long_edge;
    PPIINVZBEdge &right = handedness ? long_edge : short_edge;
...
void draw_piecewise_textured_triangle_iinvzb(RenderListPoly &poly) {
    ...
    PPIINVZBEdge bottom_to_top = PPIINVZBEdge(poly.trans_verts[v0], poly.trans_verts[v2]);

    if (Math::f_cmp(poly.trans_verts[v0].v.y, poly.trans_verts[v1].v.y)) {
        PPIINVZBEdge bottom_to_middle = PPIINVZBEdge(poly.trans_verts[v0], poly.trans_verts[v1]);

        scan_edges(bottom_to_top, bottom_to_middle, handedness, poly.color, poly);
    }
    else if (Math::f_cmp(poly.trans_verts[v1].v.y, poly.trans_verts[v2].v.y)) {
        PPIINVZBEdge middle_to_top = PPIINVZBEdge(poly.trans_verts[v1], poly.trans_verts[v2]);
